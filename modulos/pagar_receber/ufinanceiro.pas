...
begin
        ...
        if Pred(dbgrid1.SelectedRows.Count) < 0 then begin
           if not msg('Você não fez uma seleção. Deseja enviar para todos listados?',2) then abort;
        end else
        ...
        if rpg.ItemIndex<>1 then  msg('#Escolha não pago.',0);
...
begin
            ...
            imp := msgi('Entre com o período(Pagar em):',3);
...
begin
       ...
       if Pred(dbgrid1.SelectedRows.Count) < 0 then msg('#Você precisa selecionar (CTRL) a(s) parcela(s)!',0);
...
var
f,fp,sq5,sq15,sq25:tzquery;
anomes,ultdia:string;
lst:tstringlist;
        ...
        begin
                ...
                if not result then result := fp.Locate('codigo',codigo,[]);
        ...
        begin
              if (sq5.Locate('codigo',codigo,[])) and (sq5.FieldByName('valor').AsFloat >= valor)   then
                 result := strtoint(formatdatetime('dd',sq5.fieldbyname('dtvencimento').AsDateTime)) else

              if (sq15.Locate('codigo',codigo,[])) and (sq15.FieldByName('valor').AsFloat >= valor) then
                 result := strtoint(formatdatetime('dd',sq15.fieldbyname('dtvencimento').AsDateTime)) else

              if (sq25.Locate('codigo',codigo,[])) and (sq25.FieldByName('valor').AsFloat >= valor) then
                 result := strtoint(formatdatetime('dd',sq25.fieldbyname('dtvencimento').AsDateTime)) else
                 result :=10;

              if DiaSemana(strtodate(inttostr(result)+'/'+formatdatetime('mm/yyyy',date))) = 'SABADO'  then result := result + 2;
              if DiaSemana(strtodate(inttostr(result)+'/'+formatdatetime('mm/yyyy',date))) = 'DOMINGO' then result := result + 1;
...
begin                 
        senhagerencial('Financeiro','DIGITE A SENHA GERENCIAL:', 'LANÇAR MENSALIDADES');
       ...
       //cliente
        with fdm.Query1 do begin
             ...
             while not eof do begin
                   caption :='ANALISANDO/LANÇANDO, ESPERE...     ' + inttostr(fdm.Query1.RecNo)+ ' DE '+inttostr(fdm.Query1.RecordCount);
                   update;
                   if not lancado(fieldbyname('codigo').AsString) then lanca;
...
begin
        ...
        fdm.RLBRemessa1.LayoutArquivo := laCNAB400; // Layout do arquivo, é necessário ver com o banco qual é o indicado.CNAB400 é o mais comum.
...
begin
        ...
        if rpg.ItemIndex <> 1 then msg('#Apenas os não pagos podem ser alterados.',0);
        if Pred(dbgrid1.SelectedRows.Count) < 0 then msg('#Você precisa selecionar (CTRL) a(s) parcela(s)!',0);
