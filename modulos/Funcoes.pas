...
function EnDecrypt(StrValue : String) : String; // Gleyber - 03/07/2012
Function Crypt(Action, Src: String): String;  // Gleyber - 14/07/2012
...
procedure FazQuery(script:string);
begin
  fdm.sqlPub.SQL.Clear;
  fdm.sqlPub.SQL.Add(script);
   //debugstr(sqlpub);
   if (lowercase(copy(script,1,6)) = 'select') or
      (lowercase(copy(script,1,4)) = 'show') then
     fdm.sqlpub.open
...
// Gleyber - 03/07/2012 - Início 
function EnDecrypt(StrValue : String) : String;
var
  i, TamanhoString, pos, PosLetra, TamanhoChave: Integer;
begin
  Result := StrValue;

  TamanhoString := Length(StrValue);
...
begin
  ...
  Key := 'YUQL23KL23DF90WI5E1JASISTEMACXXL6JAOAUWWMCL0AOMM4PAGPOPYW9KHJUI23TIMHJKDF3424SKGLEYBERSL9RTIKJ';
  Dest := '';
  KeyLen := Length(Key);
  KeyPos := 0;
  SrcPos := 0;
