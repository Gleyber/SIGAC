...
uses  udm, funcoes, umanu,  uconsumotexto,
   uimp, ufiltrocad,UvalidaIE, uclientevalor1;
...
begin
        ...
        sqlcli.SQL.Clear;
        sqlcli.SQL.Add('select * from tbcliente where flagsite is not null');
        tabela.Close;
        tabela.Open;
        formatarexecute(self);
       ...
       // showmessage(inttostr(tabela.RecordCount));
        ...
        while not tabela.Eof do begin
              ...
              with fdm.snet do begin
                   ...
                   if not fdm.tbnet2.Locate('cpf',tabela.fieldbyname('cnpj').AsString,[]) then
                      ...
                      sql.Add('insert into tab_clientes( '+
                              ...
                              ','+quotedstr(tipo)+
                              ','+ quotedstr(tabela.fieldbyname('nome').AsString)+
                              ','+ quotedstr(inttostr(tabela.fieldbyname('codigogp').asinteger))+
                              ','+ quotedstr(cpf)+
                              ','+ quotedstr(rg)+
                              nascimento+
                      ...
                      sql.Add('update tab_clientes set '+
                                ...
                                ',codigogrupo='+ quotedstr(inttostr(tabela.fieldbyname('codigogp').Asinteger))+
                                ',cpf='+ quotedstr(cpf)+
                                ',rg ='+ quotedstr(rg)+
              ...
              caption := '..... ATUALIZANDO PARA O SITE ... ESPERE...      ' +
...
begin
        ...
        while not tabela.Eof do begin
              ...
              if fdm.snet.Locate('cod_user',tabela.fieldbyname('codigo').AsString,[]) then begin
