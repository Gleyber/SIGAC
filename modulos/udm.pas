...
type
  Tfdm = class(TDataModule)
    ...
    nfe: TACBrNFe;
    tbquery1chave: TIntegerField;
    tbquery1dtemissao: TDateField;
    tbquery1dtvencimento: TDateField;
    tbquery1dtpagamento: TDateField;
    tbquery1total: TFloatField;
    tbquery1cheque: TStringField;
    tbquery1conta: TStringField;
    tbquery1codigo: TIntegerField;
    tbquery1nome: TStringField;
    tbquery1banco: TStringField;
    tbquery1agencia: TStringField;
    ...
    procedure tbnotaitemBeforeEdit(DataSet: TDataSet);
...
uses funcoes, MMSystem, ugrafico,umail, umanu, uimp, ucadg, umovimento,
     uparcelado, uecf_,uecf_aux_,uecf_imp_, uipedido, principal, unota;
 ...
 exp:string;
      ...
      begin
          ...
          if responsavel = 'DISK BATERIAS' then
             imp := msgi('1=Imprimir cupom fipedido.tb'+#13+'2=Imprimir cupom entrega',2);

          if imp = '2' then
            periodo2('Tabdiskbateria',fipedido.tb.fieldbyname('chave').AsString,'','','','',366,330,1)
                      ...
                      adicionais.Parent := fipedido;
                      adicionais.Visible :=false;

                      rodape:= trichedit.Create(application);
                      rodape.Parent := fipedido;
                      rodape.Visible :=false;



                  if  ((fipedido.tb.fieldbyname('totalservico').asfloat > 0) and
                       (fipedido.tb.fieldbyname('totalpeca').asfloat > 0)) or
                       (fipedido.tb.fieldbyname('devolucao').asfloat > 0) or
                       (fipedido.tb.fieldbyname('descontoreal').asfloat > 0) or
                       (fipedido.tb.fieldbyname('acrescimo').asfloat > 0) then
                       total:='total'
                       else total:='';


                if tipoempresa = 'A' then  begin
                  adicionais.Lines.Add(COMPSTR('Veiculo: ' +  fipedido.tb.FieldByName('campo1').AsString,
                                       'Frota: ' + fipedido.tb.FieldByName('campo5').AsString));
                  adicionais.Lines.Add(COMPSTR('Chassi: ' + fipedido.tb.FieldByName('campo4').AsString,
                                       'Ano/Marca: ' + fipedido.tb.FieldByName('campo2').AsString));

                  adicionais.Lines.Add('Km: ' + fipedido.tb.FieldByName('campo6').AsString +
                                     '  M.V.S./Cor: ' + fipedido.tb.FieldByName('campo3').AsString +
                                     '  Placa: ' + fipedido.tb.FieldByName('campo8').AsString);

                if fipedido.tb.FieldByName('campo9').AsInteger >0 then
                   adicionais.Lines.Add('Garantia: ' + fipedido.tb.FieldByName('campo9').AsString +
                                        ' Dias >> ' + datetostr(fipedido.tb.FieldByName('data').AsDateTime +
                                        fipedido.tb.FieldByName('campo9').AsInteger));

                  adicionais.Lines.Add('Data da entrega: ' + fipedido.tb.FieldByName('dtentrega').AsString);


               end  else if tipoempresa = 'I' then  begin
                    adicionais.Lines.Add(COMPSTR('Maquina: ' + fipedido.tb.FieldByName('campo1').AsString,
                                         'Serie: ' + fipedido.tb.FieldByName('campo5').AsString));

                    adicionais.Lines.Add(COMPSTR('Cont Ini: ' + fipedido.tb.FieldByName('campo2').AsString,
                                        'Cont Fin: ' + fipedido.tb.FieldByName('campo6').AsString));

                    adicionais.Lines.Add(COMPSTR('Inicio: ' + fipedido.tb.FieldByName('campo3').AsString,
                                         'Termino: ' + fipedido.tb.FieldByName('campo5').AsString));

              end  else if tipoempresa = 'E' then  begin
                   adicionais.Lines.Add('Aparelho: ' + fipedido.tb.FieldByName('campo1').AsString);
                   adicionais.Lines.Add('Marca: ' + fipedido.tb.FieldByName('campo2').AsString);
                   adicionais.Lines.Add('Modelo: ' + fipedido.tb.FieldByName('campo3').AsString);
                   adicionais.Lines.Add('Serie: ' + fipedido.tb.FieldByName('campo4').AsString);

              end  else if tipoempresa = 'P' then  begin
                   adicionais.Lines.Add('Nome Animal: ' +
                   fipedido.tb.FieldByName('campo7').AsString + ' ' +
                   fipedido.tb.FieldByName('campo1').AsString);
                   adicionais.Lines.Add('Raca: ' + fipedido.tb.FieldByName('campo4').AsString);
                   adicionais.Lines.Add('Peso: ' + fipedido.tb.FieldByName('campo6').AsString);
                   adicionais.Lines.Add('Altura: ' + fipedido.tb.FieldByName('campo2').AsString);
                   adicionais.Lines.Add('Especie: ' + fipedido.tb.FieldByName('campo5').AsString);
                   adicionais.Lines.Add('Cor: ' + fipedido.tb.FieldByName('campo3').AsString);
              end;
             ...
             with fdm.Query2 do begin
                  ...
                  sql.add(' select vrdocumento, dtvencimento from tbreceber '+
                          ...
                          ' and linkvenda = '+ quotedstr(fipedido.tb.fieldbyname('chavepedido').AsString)+
             ...
             try
                if fipedido.tb.fieldbyname('spedido').AsFloat >0 then
                   sped := '     SEU PEDIDO: ' + formatfloat('000000',fipedido.tb.fieldbyname('numero').AsFloat);
             except end;

             {
                if (modulo = 'AVALIACAO') or (modulo = 'CONSIGNADA') or
                  ...
                  adicionais.Lines.Add('');
                  adicionais.Lines.Add('EU, ' + copy(fipedido.tb.FieldByName('nome').AsString, 1,36));
                ...
                fipedido.tbitem_.Open;
                fipedido.tbdev_.Open;

                impcupom('CUPOM',formatfloat('000000',fipedido.tb.fieldbyname('numero').AsFloat)+ sped,
                          adicionais.Text,
                          rodape.text,
                          compstr('UN              QTD ','VR. UNIT            TOTAL'),
                          'codigo','descricao','qtd','vrunit','total',
                          'totalpeca',
                          'totalservico',
                          'devolucao',
                          'descontoreal',
                          ...
                          'vrcouvert',
                          'obs',
                          fipedido.tb ,fipedido.tbitem_ ,fipedido.tbdev_ ,fipedido.tbservico,fipedido.tbpp,fipedido.tblocacao, true,fipedido);

                  adicionais.Destroy;
                  rodape.Destroy;
                  fipedido.tbitem_.close;
                  fipedido.tbdev_.close;
               end;
...
begin
        ...
        fipedido:=tfipedido.Create(application,chave,nometb,imp);
        fipedido.imp := imp;
        fipedido.configexecute(application);
        ...
        if imp='8' then begin
           ...
           imptxt(fipedido.tb ,fipedido.tbitem_ ,fipedido.tbdev_ ,fipedido.tbservico,fipedido.tblocacao,2);

           fipedido.tbitem_.close;
           fipedido.tbdev_.close;
        end else if imp='11' then cupom

        else begin

            exp :='1';
            inputquery('Imprimir/Exportar', 'Digite uma opção:' + #13 + #13 +
                      '1 = Imprime' + #13 +
                      '2 = Exporta', exp);
           if (exp <> '1') and (exp <> '2') then exp := '1';
            ...
            if (imp='2') then begin
               ...
               if exp = '1' then begin
                  ...
                  if responsavel = '08.432.073/0001-64' then
                     fipedido.qpedido10.PreviewModal
                  else
                  ...
                  if responsavel = '08.432.073/0001-64' then
                     exportarRL(fipedido.qpedido,fipedido.tb.fieldbyname('nome').asstring,fipedido.tb.fieldbyname('email').asstring)
                  else
                     exportarRL(fipedido.qpedido,fipedido.tb.fieldbyname('nome').asstring,fipedido.tb.fieldbyname('email').asstring);
               ...
               if exp = '1' then
                     ...
                     exportarRL(fipedido.qpedidoreduzido,fipedido.tb.fieldbyname('nome').asstring,fipedido.tb.fieldbyname('email').asstring);


            end else if imp = '10' then begin

                vf:=false;
                periodo2('tabplanalmed',fipedido.tb.fieldbyname('codigovendedor').AsString,'','','','',265,498,0);
                if vf then
                   if exp = '1' then
                      fipedido.qpedido10.previewmodal

                   else
                       exportarRL(fipedido.qpedido10,fipedido.tb.fieldbyname('nome').asstring,fipedido.tb.fieldbyname('email').asstring);
                   vf:=false;
            end ;
...
{tbp = itens da nota}
{tbs = servico}
{dev=nota de devolucao}
{nnota = '' = tbnota;  nnota= '1' = tbnota1, para mais de uma empresa}
{nnota = '' = tbnota;  nnota= '1' = tbnota1, para mais de uma empresa}
{tb=venda, pedido, ou entrada}




var
nome, endereco, bairro, municipio, cep, ufcli, cnpj, ie, fone,  dadosadicionais,
freteconta, natoperacao, hsaida, modelo, serie, clifor, cfop, chave, vrunit, custo,
desconto,fil,loc,codigo, vencimento1, vendedor, pedido, tex, entrada,duplicata,parc,
chavepedido,ntbvenda,pedido_,desc,nfatura:string;
...
sq:tzquery;
Ini:TInifile;
venda,exporta,dev:boolean;
tbitem:tzquery;




function icmfuf(const mn: string): double;
var sqld:tzquery;
begin
       result :=0;

       if mn = 'AL' then result := 7;
       if mn = 'AM' then result := 7;
       if mn = 'AP' then result := 7 ;
       if mn = 'BA' then result := 7 ;
       if mn = 'DF' then result := 7 ;
       if mn = 'MA' then result := 7 ;
       if mn = 'MG' then result := 12 ;
       if mn = 'MT' then result := 7 ;
       if mn = 'PA' then result := 7 ;
       if mn = 'PI' then result := 7 ;
       if mn = 'PR' then result := 12 ;
       if mn = 'RS' then result := 12 ;
       if mn = 'SC' then result := 12 ;
       if mn = 'SP' then result := 18 ;

       sqld := tzquery.Create(application);
       with sqld do begin
            Connection := fdm.conector ;
            sql.Add('select icm from tbcliente where codigo = '+quotedstr(tbg.fieldbyname('codigo').AsString));
            open;
            if fieldbyname('icm').AsFloat >0 then result := fieldbyname('icm').AsFloat;
            destroy;
       end;

       if sq.fieldbyname('mva').AsFloat <=0 then result :=0;       

end;


function mvauf(const mn: string): double;
begin
       result :=0;

       if mn = 'AL' then result := 56.9;
       if mn = 'AM' then result := 56.9;
       if mn = 'AP' then result := 56.9 ;
       if mn = 'BA' then result := 56.9 ;
       if mn = 'DF' then result := 56.9 ;
       if mn = 'MA' then result := 56.9 ;
       if mn = 'MG' then result := 50.2 ;
       if mn = 'MT' then result := 56.9 ;
       if mn = 'PA' then result := 56.9 ;
       if mn = 'PI' then result := 56.9 ;
       if mn = 'PR' then result := 50.2 ;
       if mn = 'RS' then result := 48.4 ;
       if mn = 'SC' then result := 48.4 ;
       if mn = 'SP' then result := 40;
end;


begin
                 
       if tb='F' then nfatura := codloc;

       if tbp <> nil then
       dev := tbp.fieldbyname('qtd').AsFloat <0;
       acesso1('Nota Fiscal');
       if tbg = nil then begin
          ...
          abortaacao(nil,tbg,'chave','','','','',0);
          livrofiscal(date);

          if lowercase(tb)='entrada' then begin
               clifor := 'tbfornecedor';
               entrada := 'E';
          end else begin
               clifor := 'tbcliente';
               entrada := 'S';
              // if dev then entrada := 'E';

               //dpavidros
               if responsavel = '10.413.451/0001-04' then begin

                   tbitem :=  tzquery.Create(application);

                   if lowercase(tb)='venda' then ntbvenda := 'venda'
                                            else ntbvenda := 'tbpedido';

                   chavepedido := tbg.fieldbyname('chave').AsString;
                   with tbitem do begin
                        tbitem.Connection := fdm.conector;
                        ...
                        sql.add( 'select tbproduto.codigo,tbproduto.codigooriginal,tbproduto.ncmsh,'+
                                 ...
                                 ' tbproduto.cst,'+
                                 ' tbproduto.nacimp,'+
                                 ' tbproduto.cf,tbproduto.st, ' +
                                 'sum('+ntbvenda+'item.qtd) as qtd '+
                                 ' from tbproduto, '+ntbvenda+'item'+
                                 ' where tbproduto.codigo = '+ ntbvenda+'item.codigo '+
                                 ' and  '+ntbvenda+'item.qtd > 0  and ' +
                                 ntbvenda+'item.link='+quotedstr(chavepedido)+
                                 ' group by '+ntbvenda+'item.codigo, '+
                                 ntbvenda+'item.vrunit, '+ ntbvenda+'item.descontoreal');
            ...
            sq := tzquery.Create(application);
            codloc :='';
            codigo := tbg.fieldbyname('codigo').AsString;

            with sq do begin
                 Connection := fdm.conector;

                 sql.Clear;
                 sql.Add('select nome, endereco,numero, complemento, bairro, municipio, cep, uf, cnpj, ie, fone '+
                         ' from ' + clifor + ' where codigo = ' + quotedstr(codigo));
                 open;

                 ufcli:= fieldbyname('uf').AsString;
             ...
             venda := (lowercase(tb)= 'venda') or (lowercase(tb)= 'tbpedido');

             if venda then
                frt := tbg.fieldbyname('acrescimo').AsFloat;


             if (venda) and (tbg['nf'] <> null) then
                tex := 'Já foi gerada NF desta venda!!!'#13+#13;


            if msg(tex+'Deseja gerar nota fiscal? ',2) then begin

          exporta := false;
          if ufcli='' then
             exporta := msg('Esta nota é tipo exportação?',2) = true;

          if (ufempresa <> '') and ((ufcli<>'') or (exporta)) then begin


            application.Createform(tfnota,fnota);


             Ini:=TInifile.Create(ExtractFilePath(ParamStr(0)) + 'imagem\nota.ini');
             modelo := Ini.ReadString('Configmodelo', 'modelonf', '1');
             serie := Ini.ReadString('Configmodelo', 'serienfe', '1');
             ini.Free;
            ...
            if responsavel = 'CLARUS' then
               ...
               hsaida :=timetostr(now);

            //se estiver gerando de um pedido com acréscimo
            if (clifor = 'tbcliente') and (frt>0) then begin
               if msg('Inserir o acréscimo como frete?',2)= true then
                  frete := frt;
            end else frete := frt;
            ...
            freteconta:='2';
            if (frete >0) then
               inputquery('Frete', 'O frete é por conta do:' + #13 + #13 +
                            '1 = Emitente'+#13+
                            '2 = Destinatário ', freteconta);
            if (freteconta <>'1') and (freteconta <>'2') then freteconta :='2';

            //nota gerada de fatura
            if tb='F' then desconto :=  ', '+realdblstr(tbg.fieldbyname('desconto').asfloat)
                      else desconto :=  ', null ';


            if (responsavel = '12.347.804/0001-40') and(tb<>'F') then
               pedido_ := ', '+quotedstr(tbg.fieldbyname('numero').AsString)
            else pedido_ := ', null';


            if venda then begin
               vencimento1 := tbg.fieldbyname('formapgto').AsString;
               try
               vendedor    := tbg.fieldbyname('CodigoVendedor').AsString;
               except end;
            end;
            ...
            with sq do begin
                 if (ufempresa = ufcli) then begin

                    if clifor = 'tbfornecedor' then
                       cfop := '1.102'
                    else
                       cfop := '5.102';

                    if dev then cfop := '5.202';
                    ...
                    if dev then cfop := '6.202';


                 end;

                 if exporta then cfop := '7.102';


                 //subst cupom por nf
                 if tbp['cfop'] = '5.929' then  cfop := '5.929';
                 if (responsavel='47.030.085/0001-14') or //eletrica sp e termcabos
                    (responsavel='12.347.804/0001-40') then cfop := '5.405';

                 sql.Clear;
                 sql.Add('select natoperacao from tbcfop where cfop = ' + quotedstr(cfop));
                 open;
                 natoperacao := fieldbyname('natoperacao').AsString;


                 sql.Clear;
                 sql.Add('select dadosadicionaisnota,decimaisvenda,decimaiscompra from tbconfig');

                 open;
                 codloc:= inttostr(fnota.numnota);

                 if (responsavel='12.347.804/0001-40') and (tb<>'F') then
                    dadosadicionais:= fieldbyname('dadosadicionaisnota').asstring + tbg.fieldbyname('obs').AsString
                 else
                    dadosadicionais:= fieldbyname('dadosadicionaisnota').asstring;


                 if clifor = 'tbfornecedor' then decimais := FieldByName('decimaiscompra').AsInteger
                 else                            decimais := FieldByName('decimaisvenda').AsInteger;

                 close;

                 try
                 if (tb<>'F') and (tbg['uf'] = 'EX') then begin
                    municipio := 'EXTERIOR';
                    cnpj      := '';
                    IE        := '';
                 end;
                 ...
                 sql.Clear;
                 sql.Add('insert into tbnf'+fnota.emp +'(data, modo, codigo, hsaida, entrada, '+
                         ' cfop, natoperacao, freteconta, modelo, serie, percentiss, '+
                         ' vencimento1, Vendedor, pedido,nfatura, frete, nome, '+
                         ' endereco, bairro, municipio, cep, uf, cnpj, ie, fone, numero, desconto, '+
                         ' dadosadicionais) values('+
                         ...
                         ', '+quotedstr(entrada)+
                         ', '+quotedstr(cfop)+
                         ', '+quotedstr(natoperacao)+
                         ', '+quotedstr(freteconta)+
                         ', '+quotedstr(modelo)+
                         ', '+quotedstr(serie)+
                         ', '+realdblstr(fnota.percentiss)+
                         ', '+quotedstr(vencimento1)+
                         ', '+quotedstr(vendedor)+
                              pedido_+
                         ', '+quotedstr(nfatura)+
                         ', '+realdblstr(frete)+
                         ...
                         ', '+quotedstr(codloc)+
                              desconto+
                         ', '+quotedstr(dadosadicionais)+')');

                         execsql;


                         sql.Clear;
                         sql.Add('select chave from tbnf'+fnota.emp + ' where numero = ' + quotedstr(codloc));
                         open;
                        ...
                        if (venda) or (tb='F') then begin

                           if venda then fil := 'modulo = "2" and linkvenda = '+  quotedstr(tbg.fieldbyname('chavepedido').AsString)
                           else if tb='F' then  fil := 'modulo = "28" and linkvenda = '+  quotedstr(tbg.fieldbyname('numero').AsString);

                           sql.clear;
                           sql.add('select dtvencimento, diferenca from tbreceber '+
                                   ' where ch is null and '+ fil+
                                   ' order by linkvenda, dtvencimento ');
                           open;

                           duplicata :='';
                           if sq.RecordCount >0 then begin
                              while not sq.Eof do begin

                                       parc:= inttostr(sq.RecNo);

                                       if parc ='1' then
                                          duplicata := 'vencimento1 = '+ quotedstr(sq.fieldbyname('dtvencimento').AsString)
                                       else
                                          duplicata :=duplicata + ', vencimento'+ parc + '='+ quotedstr(formatdatetime('yyyy-mm-dd',sq.fieldbyname('dtvencimento').AsDateTime));

                                       duplicata := duplicata + ', valor'+ parc + '='+ realdblstr(sq.fieldbyname('diferenca').asfloat);
                                       duplicata := duplicata + ', dup'+ parc + '='+ quotedstr(formatfloat('000000',strtofloat(codloc))+'/'+parc);
                                       sq.Next;
                                    end;

                                    selecione('update tbnf' + fnota.emp + ' set '+ duplicata + ' where chave = '+quotedstr(chave));
                                    close;
                              end;
                        end;
                         ...
                         sql.Clear;
                         sql.Add('select * from tbnfitem where chave < 1000 ');
                         open;
                 end;

                 if clifor = 'tbfornecedor' then begin
                    vrunit := 'totalparcial';
                    custo := 'vrunit';
                 end else begin
                    vrunit := 'vrunit';
...
//                 try  tbp.IndexFieldNames :='item'; except end;


                 tbp.First;
                 for i := 1 to tbp.RecordCount do begin

                     //preenche os dados do item
                     sq.edit;
                     sq['link'] := chave;

                     if i>0 then sq['item'] := i;

                     if clifor <> 'tbfornecedor' then
                        desc := floattostr(tbp.FieldByName('descontoreal').AsFloat*tbp.FieldByName('qtd').AsFloat)
                     else desc :='0';

                     if  tbp.FieldByName('qtd').AsFloat < 0 then
                         sq['qtd'] := tbp.FieldByName('qtd').AsFloat*(-1)
                     else
                         sq['qtd'] := tbp.FieldByName('qtd').AsFloat;

                     sq['custo'] := tbp.FieldByName(custo).asfloat;
                     sq['codigo'] := tbp.FieldByName('codigo').AsString;

                     sq['desctotal'] := desc;

                     if lowercase(tb)<>'entrada' then
                        sq['ncmsh'] := tbp.FieldByName('ncmsh').AsString;

                     sq['codigooriginal'] :=  tbp.FieldByName('codigooriginal').AsString;
                     sq['descricao'] := tbp.FieldByName('descricao').AsString;
                     sq['un'] := tbp.FieldByName('un').AsString;

                     sq['vrunit'] := roundto((tbp.FieldByName(vrunit).AsFloat),-decimais);
//                     sq['vrtotal'] := (sq.FieldByName('vrunit').AsFloat* tbp.FieldByName('qtd').AsFloat);

                     if responsavel <> '12.347.804/0001-40' then begin
                        sq['ipi'] := tbp.FieldByName('ipi').AsFloat;
                        sq['vripi'] := tbp.FieldByName('vripi').AsFloat;
                     end;

                     if tbp['qtd'] > 0 then begin
                        if (tbp['mva'] <> null) and (tbp.FieldByName('mva').AsString <> '') then
                           sq['mva'] := tbp.FieldByName('mva').AsFloat;

                        if clifor = 'tbfornecedor' then begin
                           sq['st'] := tbp.FieldByName('CST').AsString;
                           sq['pis'] := tbp.FieldByName('vrpis').AsFloat;
                           sq['confins'] := tbp.FieldByName('vrcofins').AsFloat;
                           sq['reducaobasecalc'] := tbp.FieldByName('RedBase').AsFloat;

                        end else begin
                           sq['reducaobasecalc'] := tbp.FieldByName('reducaobasecalc').AsFloat;
                           sq['cf'] := tbp.FieldByName('cf').AsString;
                           
                           sq['st'] := tbp.FieldByName('st').AsString;
                           sq['pis'] := tbp.FieldByName('pis').AsFloat;
                           sq['confins'] := tbp.FieldByName('cofins').AsFloat;
                        end;
                     end;

                     if strisfloat(tbp.FieldByName('icm').AsString)=true then
                        sq['icm'] := tbp.FieldByName('icm').AsFloat;

                     //subst cupom por nf
                     if tbp['cfop'] = '5.929' then sq['cfop'] := '5.929'

                     else if clifor <> 'tbfornecedor' then begin
                        if (ufempresa = ufcli) then begin

                           if tbp['cfop']<>null then sq['cfop'] := tbp['cfop']
                                                else sq['cfop'] := '5.102';

                           if dev then sq['cfop'] := '5.202';

                        end else begin

                           if tbp['st'] = '060' then sq['cfop'] := '6.405'
                                                else sq['cfop'] := '6.102';

                           if dev then sq['cfop'] := '6.202';

                        end;



                     end else  sq['cfop'] := cfop;

                     if exporta then sq['cfop'] := '7.102';

                     if length(sq.FieldByName('st').AsString) <> 3 then sq['st'] := '000';

                     if length(sq.FieldByName('cfop').AsString) <> 5 then sq['st'] := '5.102';
               ...
               selecione('select nacimp, ncmsh, cst,nacimp from tbproduto where codigo = '+ quotedstr(tbp.fieldbyname('codigo').AsString));
               sq['nacimp'] := sqlpub.fieldbyname('nacimp').AsString;
               sq['ncmsh'] := sqlpub.fieldbyname('ncmsh').AsString;

               if calccusto then
                  sq['st'] := sqlpub.fieldbyname('nacimp').AsString+sqlpub.fieldbyname('cst').AsString
               else sq['st'] := sqlpub.fieldbyname('cst').AsString;
               sqlpub := nil;


               if (ufempresa <> ufcli) and ((tb='F') or (venda)) and ((sq['st'] = '010') or ('st' = '030')or ('st' = '060') or ('st' = '070') or
                  ('st' = '090') or ('st' = '201')or ('st' = '202')or ('st' = '203')or ('st' = '900')) then begin

                  try
                     sq['mva']  := mvauf(ufcli);
                  except end;

                  if sq.fieldbyname('mva').AsFloat >0 then
                     try
                        sq['icmf'] := icmfuf(ufcli);
                     except end;
               end;

               // totaliza os itens da nota
               fnota.calcitem(sq);

              grava(sq,nil,'tbnfitem'+fnota.emp);
              sq.Cancel;
          ...
          //servico
        if (tbs <> nil) and (tbs.Active) and (tbs.RecordCount>0) and (not fnota.empsimples) then begin
           tbs.First;

           for i := 1 to tbs.RecordCount do begin

               selecione('Insert into tbnfservico'+fnota.emp+'(codigo, descricao, qtd, vrunit, total, link) values  ('+
                         quotedstr(tbs.fieldbyname('codigo').AsString) +
                         ','+quotedstr(tbs.fieldbyname('descricao').AsString) +
                         ','+realdblstr(tbs.fieldbyname('qtd').asfloat) +
                         ','+realdblstr(tbs.fieldbyname('vrunit').asfloat) +
                         ','+realdblstr(tbs.fieldbyname('total').asfloat) +
                         ','+quotedstr(chave)+')');
        ...
        if tb='F' then begin
            selecione('update tbpedido set nf = '+  quotedstr(codloc) + ' where linkvenda = '+quotedstr(nfatura));


        end else if venda then begin

           selecione('update tbreceber set nf = '+  quotedstr(codloc) + ' where ' + fil);

           selecione('update '+ tb + ' set nf = '+  quotedstr(codloc) +
                     ' where numero = '+quotedstr(tbg.fieldbyname('numero').AsString));

               loc := tbg.fieldbyname('chave').AsString;
               tbg.Cancel;
               tbg.Refresh;
               tbg.Locate('chave',loc,[]);
        end;
        ...
        ///
            ...
            if ufcli='' then
                msg('A Nota Fiscal não foi gerada, não há um estado cadastrado.',0);

            if ufempresa = '' then
                msg('A Nota Fiscal não foi gerada, cadastre os dados da sua empresa.',0);
         end;
        end;
     end;
end;
...
procedure Tfdm.tbnotaitemBeforeEdit(DataSet: TDataSet);
begin
  if fnota <> nil then begin
     fnota.btnGravar.Enabled := True;
     fnota.btncancelar.Enabled := True;
  end;
end;
