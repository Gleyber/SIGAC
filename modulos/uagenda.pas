...
uses
  ...
  Dialogs, ComCtrls, StdCtrls, ExtCtrls, Spin, Grids, DBGrids, FMTBcd,
  Provider, DB, SqlExpr, DBClient, Buttons, jpeg, DBCtrls;
...
implementation
uses uabout,ubackup,funcoes, umanu, udm, usenha,ucadastrop;
{$R *.dfm}
...
//                          tradiobutton(Components[i]).Brush.Color := $0060514A;
       ...
       //programado
         with fdm.query1 do begin
              close;
              CommandText := 'select data,fim,programado from tbagenda where ' +
             ...
             //compromisso único
             close;
             CommandText := 'select  "             " as diasemana, tbagenda.* from tbagenda where ' +
                            ' (extract(year from  data) = ' + quotedstr(ano)+
             ...
             while not eof do begin
              ...
              //semanal
              close;
              CommandText := 'select data,fim,dia from tbagenda where ' +
                           // ' extract(year from  data) = ' + quotedstr(ano)+
                           // ' and extract(month from  data) = ' + quotedstr(mes1)+
...
procedure Tfagenda.SpeedButton1Click(Sender: TObject);
begin
           impgrade(tabela,ds,DBGrid1,caption,'tbagenda','');
end;

procedure Tfagenda.Timer1Timer(Sender: TObject);
begin
        Caption := 'Agenda de Compromissos   -   Hoje: '+ datetimetostr(now);
...
procedure Tfagenda.SpeedButton2Click(Sender: TObject);
var
comp:string;
begin
  ...
  if Application.MessageBox(pchar(comp),
  ...
  begin
    ...
    begin
      Close;
      commandtext := 'delete from tbagenda where chave = ' +
...
i:integer;
  ...
  {      if agenda then begin
        ...
        with fdm.query1 do begin
             close;
             commandtext := 'select nome, data,cor,dt from tbdatacomemorativa order by dt';
             open;
             ...
             while not eof do begin
                   if length(fieldbyname('data').AsString)>0 then
                      tbdc.Insert;
  ...
  if fileexists(PChar(ExtractFilePath(ParamStr(0)) + 'imagem\menu3.jpg')) then
     imagepanel('imagerodape1',alclient,Panel6, (PChar(ExtractFilePath(ParamStr(0)) +'imagem\menu3.jpg')),nil);

  if fileexists(PChar(ExtractFilePath(ParamStr(0)) + 'imagem\barra.jpg')) then
     imagepanel('imagerodape',alclient,Panel4, (PChar(ExtractFilePath(ParamStr(0)) +'imagem\barra.jpg')),nil);
...
procedure Tfagenda.StaticText1Click(Sender: TObject);
begin
              Acesso1('Cadastro');
              fcadastrop:=tfcadastrop.Create(self,'datacomemorativa','',
                      'Preencha o campo!',
                      'data','nome',true );
...
procedure Tfagenda.SpeedButton6Click(Sender: TObject);
begin
              Acesso1('Agenda de Telefones');
              fcadastrop:=tfcadastrop.Create(self,'fone','',
                      'Preencha os campos!',
                      'nome','nome',true );
              desativar(fcadastrop);
              fcadastrop.showmodal;
