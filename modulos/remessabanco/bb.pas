{
  27/07/2009 - Dione Betanin
  Alterações conforme solicitação da CSO Brasília, verificar tags 27-07-09
}
...
begin
   ...
   begin
      ...
      if LayoutNN = lnC7_N10 then
      begin
        ANossoNumero := Formatar(NossoNumero,10,false,'0');
        ADigitoNossoNumero := Modulo11(Cedente.ContaBancaria.Convenio+ANossoNumero,9);
      end
      else
      begin
...
//      lnC7_N10: ANossoNumero := Formatar(Cedente.ContaBancaria.Convenio,7,false,'0') + Formatar(NossoNumero,10,false,'0') + '-' + DigitoNossoNumero; {Removido conforme instruções de cso.curitiba.convenio@bb.com.br suporte técnico do banco do brasil
...
begin
   ...
   begin
      ...
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,18,false,'0') + '  '; //33 a 52 - Código do convênio no banco [ Alterado conforme instruções da CSO Brasília ] {27-07-09}
      ...
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,18,false,'0') + '  '; //34 a 53 - Código do convênio no banco [ Alterado conforme instruções da CSO Brasília ] {27-07-09}
      ...
      begin
         ...
         Registro := Registro + Formatar(Titulos[NumeroRegistro].NossoNumero,20,True,'0'); //38 a 57 - Nosso número - identificação do título no banco
...
//         Registro := Registro + Formatar(Titulos[NumeroRegistro].SeuNumero,15,false,'0'); //63 a 77 - Número que identifica o título na empresa
         Registro := Registro + Formatar(Titulos[NumeroRegistro].SeuNumero,10,false,'0') + '00000'; //63 a 72 - Número que identifica o título na empresa [ Alterado conforme instruções da CSO Brasília ] {27-07-09}
         ...
         //**********************  Codigo p/ Baixa/Devolucao  ******************
{
         ...
         Registro := Registro + ' '; //224 - Campo não tratado pelo BB [ Alterado conforme instruções da CSO Brasília ] {27-07-09}
         Registro := Registro + '   '; //225 a 227 - Campo não tratado pelo BB [ Alterado conforme instruções da CSO Brasília ] {27-07-09}
          //********************************************************************
...
begin
  ...
  begin
    ...
    Remessa.Add('0'+ // IDENT.DO REGISTRO
                ...
                Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,8,false,'0')+ // IDENT.DO CEDENTE NO BANCO
                Formatar(Titulos[NumeroRegistro].Cedente.DigitoCodigoCedente,1,false,'0')+ // IDENT.DO CEDENTE NO BANCO
                '000000'+
                ...
                FormatDateTime('ddmmyy',DataArquivo)+ // DATA DA GRAVAÇÃO DO ARQUIVO
                Formatar(IntToStr(NumeroArquivo),7,false,'0')+ //NUMERO DE GERACAO DO ARQUIVO
                Formatar('',22)+ // Brancos
                Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.Convenio,7,False,'0')+ //Número do Convênio
                Formatar('',258)+ // Brancos
...
//          toRemessaAlterarSeuNumero          : ATitulosTipoOcorrencia := '08';
      ...
      if Titulos[NumeroRegistro].LayoutNN = lnC7_N10 then
        Registro := '7'
      else
      ...
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,8,False,'0'); // IDENT.DO CEDENTE NO BANCO
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.DigitoCodigoCedente,1,False,'0'); // IDENT.DO CEDENTE NO BANCO
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.Convenio,7,False,'0'); //Convênio
      Registro := Registro + Formatar(Titulos[NumeroRegistro].NumeroDocumento,25,True,' '); //Número de Controle - Livre
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.Convenio,7,False,'0')+Formatar(Titulos[NumeroRegistro].NossoNumero,10,False,'0'); // Nosso Número
      Registro := Registro + '00'; // Número da Prestação
      Registro := Registro + '00'; // Indicativo de Grupo
      Registro := Registro + '   '; // Brancos
      Registro := Registro + ' '; // Brancos
      case StrToIntDef(Titulos[NumeroRegistro].Carteira,0) of
        31,51: Registro := Registro + 'SD ';
        12   : Registro := Registro + 'AIU';
      else
        Registro := Registro + 'AI '; // Prefixo do Titulo
      end;
      Registro := Registro + '019'; // Display Variação da Carteira
      Registro := Registro + '0'; // Display Conta Caução
      Registro := Registro + '000000'; // Display Número Borderô
      Registro := Registro + '     '; // BRANCOS
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Carteira,2); // Código da Carteira
      ...
      begin
        Registro := Registro + Formatar(FloatToStr(Titulos[NumeroRegistro].DataProtesto - Titulos[NumeroRegistro].DataVencimento),2,false,'0');
         ...
         begin
            ...
            begin
               ...
               begin
                  ...
                  NossoNumero := Copy(Retorno.Strings[NumeroRegistro],38,17);
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        if Copy(Retorno.Strings[NumeroRegistro],1,1) = '7' then //Tipo de Registro
        begin
          ...
          begin
            ...
            ValorOutrosCreditos  := StrToFloat(Copy(Retorno.Strings[NumeroRegistro],280,13))/100;
            ValorRecebido        := StrToFloat(Copy(Retorno.Strings[NumeroRegistro],254,13))/100;
            ValorDespesaCobranca := StrToFloat(Copy(Retorno.Strings[NumeroRegistro],182,7))/100;

            {Dados que variam de acordo com o banco}
            {Nosso número SEM DÍGITO}
            NossoNumero := Copy(Retorno.Strings[NumeroRegistro],71,10);
            Cedente.ContaBancaria.Convenio      := Copy(Retorno.Strings[NumeroRegistro],64,7);
            Cedente.ContaBancaria.CodigoAgencia := Copy(Retorno.Strings[NumeroRegistro],18,4);
            Cedente.ContaBancaria.DigitoAgencia := Copy(Retorno.Strings[NumeroRegistro],22,1);
            ...
            case StrToInt(ATipoOcorrencia) of
              ...
              03: TipoOcorrencia := toRetornoComandoRecusado;
              05: TipoOcorrencia := toRetornoLiquidado;
              06: TipoOcorrencia := toRetornoLiquidado;
              07: TipoOcorrencia := toRetornoLiquidado;
              ...
              16: TipoOcorrencia := toRetornoOutrasOcorrencias; //Confirmação de alteração de juros de mora
              19: TipoOcorrencia := toRetornoRecebimentoInstrucaoProtestar;
              20: TipoOcorrencia := toRetornoDebitoEmConta;
              21: TipoOcorrencia := toRetornoNomeSacadoAlterado;
              22: TipoOcorrencia := toRetornoEnderecoSacadoAlterado;
              23: TipoOcorrencia := toRetornoEncaminhadoACartorio;
              24: TipoOcorrencia := toRetornoProtestoSustado;
              25: TipoOcorrencia := toRetornoJurosDispensados;
              28: TipoOcorrencia := toRetornoManutencaoTituloVencido;
              31: TipoOcorrencia := toRetornoDescontoConcedido;
              32: TipoOcorrencia := toRetornoDescontoCancelado;
              33: TipoOcorrencia := toRetornoDescontoCancelado;
              34: TipoOcorrencia := toRetornoOutrasOcorrencias; //Alterar data para Desconto
              35: TipoOcorrencia := toRetornoOutrasOcorrencias; //Cobrar Multa
              37: TipoOcorrencia := toRetornoOutrasOcorrencias; //Dispensar Multa
              38: TipoOcorrencia := toRetornoOutrasOcorrencias; //Dispensar Indexador
              39: TipoOcorrencia := toRetornoOutrasOcorrencias; //Dispensar prazo limite para recebimento
              72: TipoOcorrencia := toRetornoTipoCobrancaAlterado;
              96: TipoOcorrencia := toRetornoDespesasProtesto;
              97: TipoOcorrencia := toRetornoDespesasSustacaoProtesto;
              98: TipoOcorrencia := toRetornoDebitoCustasAntecipadas;
            else
              TipoOcorrencia := toRetornoOutrasOcorrencias;
            end; {case StrToInt(ATipoOcorrencia)}
