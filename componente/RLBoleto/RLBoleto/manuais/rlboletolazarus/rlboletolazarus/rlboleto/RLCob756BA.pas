...
uses
  ...
  {$ELSE}
    {$IFDEF VER130}
      ,Mask, contnrs
    {$ELSE}
      , MaskUtils, contnrs, DateUtils
    {$ENDIF}
  {$ENDIF};

const
  CodigoBanco = '756'; //Bahia
  NomeBanco = 'BANCOOB';
  Imagem = 18;

type
  TRLBanco756 = class(TPersistent)
  private
{$IFNDEF VER120}
...
function TRLBanco756.GetImagem : Byte;
begin
  Result := Imagem;
end;

function TRLBanco756.GetNomeBanco : string;
begin
  Result := NomeBanco;
end;

function TRLBanco756.CalcularDigitoNossoNumero(ATitulo: TRLBTitulo) : string;
var
  ACodigoAgencia,
  ANossoNumero,
  ACodigoCedente,
...
begin
  ...
  begin
    ...
    ADigitoCedente     := Formatar(Cedente.DigitoCodigoCedente,1,false,'0');
    ANossoNumero       := Formatar(NossoNumero,7,false,'0');
    ADigitoNossoNumero := ACodigoAgencia+ACodigoCedente+ADigitoCedente+ANossoNumero;
  end;
  Soma := 0;
  Peso := 1;
  Constante := '3791';
  for Contador := Length(ADigitoNossoNumero) downto 1 do
  begin
    Soma := Soma + (StrToInt(ADigitoNossoNumero[Contador]) *
                    StrToInt(Constante[Peso]));
    if Peso < 4 then
      Peso := Peso + 1
    else
      Peso := 1;
  ...
  if (Digito = 0) or (Digito = 1) or (Digito>9) then
    Digito := 0;
  Result := IntToStr(Digito);
end;

function TRLBanco756.GetCampoLivreCodigoBarra(ATitulo: TRLBTitulo) : string;
var
  AAnoAtual,
  ACarteira,
  ANossoNumero,
...
begin
  with ATitulo do
  begin
    ACarteira          := Formatar(IntToStr(StrToInt(Carteira)),1);
    ACodigoAgencia     := Formatar(Cedente.ContaBancaria.CodigoAgencia,4,false,'0');
    AModalidade        := '01';
  ...
  Result := ACarteira+ACodigoAgencia + AModalidade + ACodigoCedente +
            ADigitoCedente + ANossoNumero + ADigitoNossoNumero + ANumParcela;
end;

procedure TRLBanco756.FormatarBoleto(ATitulo: TRLBTitulo; var AAgenciaCodigoCedente, ANossoNumero, ACarteira, AEspecieDocumento: string);
begin
  with ATitulo do
  begin
    AAgenciaCodigoCedente :=
       Formatar(Cedente.ContaBancaria.CodigoAgencia,4,false,'0') + ' / ' +
       Formatar(Cedente.CodigoCedente,6,false,'0') + '-' + Cedente.DigitoCodigoCedente;
    ANossoNumero := Formatar(NossoNumero,7,false,'0') + '-' + DigitoNossoNumero;
    ACarteira := Carteira;
...
function TRLBanco756.GerarRemessaCNAB240(var ACobranca: TRLBRemessa; var Remessa: TStringList) : boolean;
var
...
begin
  ...
  begin
    ...
    Registro := Registro + Formatar('BANCOOBCED',30,true,' '); //103 a 132 - Nome do banco
...
function TRLBanco756.GerarRemessaCNAB400(var ACobranca: TRLBRemessa; var Remessa: TStringList) : boolean;
var
...
begin
  ...
  begin
    ...
    Remessa.Add('0'+ // IDENT.DO REGISTRO
                ...
                Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.CodigoAgencia,4,false,'0')+ //AG ONDE O CLIENTE MANTÉM C/C
                ...
                Formatar(Titulos[NumeroRegistro].Cedente.DigitoCodigoCedente,1,false,' ')+ // IDENT.DO CEDENTE NO BANCO
                Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.NumeroConta,6,false,'0')+ // Conta Corrente
                ...
                Formatar('BANCOOBCED',15,true,' ')+ // NOME DO BANCO POR EXTENSO
    ...
    begin
      ...
      Registro := Registro + Formatar(Titulos[NumeroRegistro].NossoNumero,11,false,'0'); // Nosso Numero
      Registro := Registro + Titulos[NumeroRegistro].DigitoNossoNumero; // Digito Nosso Numero
      Registro := Registro + '00'; // Número da Parcela
      Registro := Registro + '00'; // Indicativo de valor de grupo
      Registro := Registro + '   '; // Brancos
      Registro := Registro + ' '; //Branco
      Registro := Registro + Formatar(aTitulosEspecieDocumento,3,true,' '); //Espécie do Título
      Registro := Registro + '000'; // Variação
      Registro := Registro + '0'; // Conta Caução
      Registro := Registro + '000000'; // Contrato
      Registro := Registro + '000000'; // Borderô
      Registro := Registro + '     '; //Brancos
...
function  TRLBanco756.GerarRemessa(var ACobranca: TRLBRemessa; var Remessa: TStringList) : boolean;
begin
...
function TRLBanco756.LerRetornoCNAB240(var ACobranca: TRLBRemessa; Retorno: TStringList) : boolean;
var
  ACodigoBanco,
  ANomeCedente,
  ATipoInscricao,
...
function TRLBanco756.LerRetornoCNAB400(var ACobranca: TRLBRemessa; Retorno: TStringList) : boolean;
var
  ACodigoBanco,
  ANomeCedente,
  ATipoInscricao,
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          DataVencimento := EncodeDate(StrToInt('20'+Copy(Retorno.Strings[NumeroRegistro],151,2)),
          ...
          ValorDespesaCobranca := StrToFloat(Trim(Copy(Retorno.Strings[NumeroRegistro],182,7)))/100;
          DataCredito := EncodeDate(StrToInt('20'+Copy(Retorno.Strings[NumeroRegistro],180,2)),
                         StrToInt(Copy(Retorno.Strings[NumeroRegistro],178,2)),
                         StrToInt(Copy(Retorno.Strings[NumeroRegistro],176,2)));
...
function TRLBanco756.LerRetorno(var ACobranca: TRLBRemessa; Retorno: TStringList) : boolean;
var
  ACodigoBanco,
  ANomeCedente,
...
function TRLBanco756.VerificaOcorrenciaOriginal(sOcorrenciaOriginal: String): String;
begin
...
function TRLBanco756.VerificaMotivoRejeicaoComando(sMotivoRejeicaoComando: String): String;
begin
