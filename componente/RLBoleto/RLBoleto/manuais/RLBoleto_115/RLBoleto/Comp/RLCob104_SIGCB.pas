...
Apenas converti a unit do Delphi-Boleto para o RLBoleto,
uma vez que ambos posseum praticamente a mesma estrutura.
...
begin
   ...
   ANossoNumero := '24' + Formatar(ATitulo.NossoNumero, 15, false, '0');
...
var
   ANossoNumero,
   ACodigoAgencia,
   ACodigoCedente, ACarteira: string;
begin
   ...
   with ATitulo do
   begin
     if (Carteira = '01') or (Carteira = '1') then
       ACarteira := '14'
     else
       ACarteira := '24';

      ANossoNumero := ACarteira + Formatar(NossoNumero,15,false,'0');
      ACodigoCedente := Formatar(Cedente.CodigoCedente, 6, false, '0');
   end;

   Result := ACodigoCedente + ATitulo.Cedente.DigitoCodigoCedente +
             Copy(ANossoNumero, 3, 3) + ACarteira[1] +
             Copy(ANossoNumero, 6, 3) + ACarteira[2] +
             Copy(ANossoNumero, 9, 9);
   Result := Result + Modulo11(Result, 9);
end;
...
begin
   with ATitulo do
   begin
      AAgenciaCodigoCedente := Formatar(Cedente.ContaBancaria.CodigoAgencia,4,false,'0') + '/' + Formatar(Cedente.CodigoCedente,6,false,'0') + '-' + Cedente.DigitoCodigoCedente;

      if (Carteira = '01') or (Carteira = '1') then
        ANossoNumero := '14/'
      else
        ANossoNumero := '24/';
      ANossoNumero := ANossoNumero + Formatar(NossoNumero, 15, false, '0') + '-' + DigitoNossoNumero;
