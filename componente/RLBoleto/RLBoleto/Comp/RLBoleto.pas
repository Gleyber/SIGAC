{ @abstract(Unit principal dos componentes @link(TRLBTitulo) e @link(TRLBRemessa)
            responsáveis pela emissão de boletos, arquivos remessa e retorno.)
  @author(Weber Damasio)
  @author(Fabio Henrique Soares Pita)
  @author(Marcos Rocha)
  @author(O componente RLBoleto foi criado com referência e base no componente de
          código aberto Delphi-Boleto de Genilton Barbosa <delphi-boleto@yahoogrupos.com.br>)
  @created(14 de Janeiro de 2004)

  A unit RLBoleto.pas centraliza todo controle para emissão de boletos de cobrança além dos
  arquivos remessa e retorno.
  Os componentes @link(TRLBTitulo) e @link(TRLBRemessa) nos permite emitir controlar boletos de cobrança de
  praticamente qualquer instituição financeira do mercado.

  Cada banco é controlado com sua própria unit devidamente nomeada por RLCob mais o código do
  banco. (RLCob000.pas). Como cada banco tem sua própria unit podemos gerenciar separadamente
  as particularidades de cada instituição sem interferir diretamente nos componentes principais.
...
uses
   ...
   BoletoLayout, Controls, Forms, DBClient, MidasLib, DB, StrUtils,
   RLBLayout
  {$IFDEF VER150}
...
type
{ @abstract(Classe para gerar código de barras para boletos) }
   TRLCobCodBar = class
   private
      {Dados que serão incluídos no código de barras: Número do banco, Código da Moeda,
       Valor, Campo Livre}
      fCodigo: string;
      {Retorna a representação numérica do código de barras.
       A linha digitável é baseada nas informações do código de barras. As informações que fazem parte do código de barras são:
       @longcode(
       Posição         Conteúdo
      ...
      {Define o formato do código de barras INTERCALADO 2 DE 5, retornando a seqüência de 0 e 1 que será usada para gerar a imagem do código de barras}
      function Define2de5 : string;
      {Retorna a imagem do código de barras em formato Bitmap}
   ...
   TCEP = string[8];
   {
   @abstract(Representa o endereço de cedentes ou sacados)
   }
   ...
   {
   @abstract(Informações sobre o banco)
   }
   TRLBanco = class(TPersistent)
   private
     {Código do banco na câmara de compensação}
      fCodigo : string;
      {Index da imagem no componente TImageList da unit BoletoLayout.pas}
      fImagem : Byte;
      procedure SetCodigo(ACodigoBanco : string);
      {Retorna o dígito do código do banco}
   ...
   {
   @abstract(Dados da conta bancária de cedentes ou sacados)
   Conta bancária para sacados apenas é utilizado quando é necessário autorizar o débito
   automático do título em questão.
   }
   TRLContaBancaria = class(TPersistent)
   public
      {Banco onde o cedente ou sacado tem a conta}
      fBanco : TRLBanco;
      fCodigoAgencia,
      fDigitoAgencia,
      fNumeroConta,
      fDigitoConta,
      {Utilizado quando trabalhando com o Banco do Brasil (001), utilizado nas carteiras 16, 17, 18}
      fConvenio: String;
      fNomeCliente: String;
      constructor Create;
   ...
   {
   @abstract(Dados sobre os cedentes ou sacados)
   }
   ...
   {
   @abstract(Dados completos sobre o cedente)
   Classe herdada de @link(TRLPessoa)}
   TRLCedente = class(TRLPessoa)
   private
      {Conhecido também como Código de Cobrança, pode ou não ser o proprio número da conta,
       verifique no manual de cobrança do banco para maiores informações}
   ...
   {Especifica o tipo de documento que deu origem ao título}
   TEspecieDocumento =
   (
      edAluguel,
      edApoliceSeguro,
   ...
   TLayoutNN = ({Nosso Número com 11 digitos}
                ...
                lnC6_N5);
   {Define o formato do boleto a ser impresso} 
   TBoletoLayout = (blPadrao, blCarne);
   {
   @abstract(Representa um título (boleto) e todas as rotinas associadas)
   }
   TRLBTitulo = class(TComponent)
   private
      {Form com layout do boleto a ser impresso}
      FBoletoLayout: TBoletoLayout;
      {Indica se será ou não apresentado o Print Dialog no momento da impressão}
      FPrintDialog: Boolean;
      fTipoOcorrencia : TTipoOcorrencia;
      {Indica o código da ocorrência no banco, consulte o manual de cobrança do banco para maiores informações}
      fOcorrenciaOriginal: String;
      fDescricaoOcorrenciaOriginal : String;
      {Ao ler arquivos retorno e no caso de título rejeitado indica o código do motivo de rejeição,
       para maiores informações consulte o manual de cobrança do banco}
      fMotivoRejeicaoComando : String;
      fDescricaoMotivoRejeicaoComando : string;
      {Aquele que emitiu o título}
      fCedente : TRLCedente;
      {Devedor}
      fSacado : TRLPessoa;
      {Local onde o título deverá ser pago}
      fLocalPagamento: String;
      {Número que identifica o título na empresa}
      fSeuNumero: String;
      {Número que identifica o título no banco}
      fNossoNumero: String;
      {Número do documento que gerou o título (número da nota fiscal, por exemplo)}
      fNumeroDocumento: String;
      {Carteira do título, conforme informado pelo banco}
      fCarteira : String;
      fAceiteDocumento : TAceiteDocumento;
      fEspecieDocumento: TEspecieDocumento;
      {Data em que o boleto bancário foi gerado}
      fDataProcessamento: TDateTime;
      {Data da emissão do documento que gerou o título (data da emissão da nota fiscal, por exemplo)}
      fDataDocumento: TDateTime;
      {Data do vencimento do título}
      fDataVencimento: TDateTime;
      {Data da ocorrência em questão (pagamento do título, recebimento de instrução, etc)}
      fDataOcorrencia: TDateTime;
      {Data em que o banco liberará o crédito para o cedente}
      fDataCredito: TDateTime;
      {Data até a qual deverá ser concedido abatimento}
      fDataAbatimento: TDateTime;
      {Data até a qual deverá ser concedido desconto}
      fDataDesconto: TDateTime;
      {Data a partir da qual deverão ser cobrados juros / mora}
      fDataMoraJuros: TDateTime;
      {Data em que o título deverá ser protestado em caso de falta de pagamento}
      fDataProtesto: TDateTime;
      {Data em que o título deverá ser baixado. @bold(Data de baixa não é a mesma coisa que Data de Pagamento.)

       A data de baixa é utilizada para indicar que o título teve seu pedido de registro cancelado.
      }
      fDataBaixa: TDateTime;
      {Valor do título}
      fValorDocumento: Currency;
      {Valor que o banco cobrou pelo serviço de cobrança}
      fValorDespesaCobranca: Currency;
      {Valor do abatimento a conceder / concedido ao sacado}
      fValorAbatimento: Currency;
      {Valor do desconto diário a conceder (remessa) ou desconto total concedido (retorno) ao sacado}
      fValorDesconto: Currency;
      {Valor dos juros / multa cobrados do sacado}
      ...
      {Valor de outros créditos que o banco repassará ao cedente}
      fValorOutrosCreditos: Currency;
      {Valor total recebido informado pelo banco}
      fValorRecebido: Currency;
      fReferencia: ShortString;
      {Instruções incluídas no título}
      fInstrucoes : TStringList;
      {Indica quem emite o boleto: banco ou cliente}
      fEmissaoBoleto: TEmissaoBoleto;
      {Tipo do layout do nosso numero}
      fLayoutNN: TLayoutNN;
      {Imagem da empresa no boleto}
      fImagemEmpresa: TRLImage;
      {Controla o número de titulos em preview de impressão em lote}
      nQtdeBoletos : Integer;
      {Array de títulos do lote o preview de impressão}
      ...
      {Retorna um objeto do tipo TRLCobCodBar contendo linha digitável e imagem do código de barras baseados nos dados do título
       ...
       A segunda parte do código de barras é um campo livre, que varia de acordo com o banco
       ) }
      function   GerarCodigoBarra : TRLCobCodBar;
      {Calcula o dígito do NossoNumero, conforme critérios definidos por cada banco}
      function   CalcularDigitoNossoNumero : string;
      {Atribui valores aos campos do boleto antes que ele seja impresso }
      procedure  PrepararBoleto(ABoleto: TLayoutBoleto); deprecated;
      {Gera a imagem do boleto}
      function   GetImagemBoleto : TImage;
      {Gera a imagem da ficha de compensação}
      function   GetImagemFichaCompensacao : TImage;
   public
      ID: String;
      constructor Create(AOwner: TComponent); override;
      destructor  Destroy; override;
      procedure   Assign(ATitulo: TRLBTitulo); reintroduce;
      {Preview de Impressão do título}
      procedure   Visualizar; deprecated;
      procedure PreviewModal(num:integer);
      {Prepara o Preview de Impressão em lote dos títulos}
      procedure   Preparar; deprecated;
      {Imprime o título direto para a impressora @seealso fPrintDialog}
      procedure   Imprimir; deprecated;
      { Imprime o boleto direto para a impressora @seealso fPrintDialog}
      procedure Print;
      {Insere o boleto em um CDS para que posso ser impresso em lote }
   ...
   published
      property BoletoLayout: TBoletoLayout read FBoletoLayout write FBoletoLayout default blPadrao;
      ...
      property Versao: ShortString read FVersao write SetVersao;
   end;

{$IFNDEF VER120}

   {
   @abstract(Representa uma lista de títulos - Objetos do tipo @link(RLBTitulo))
   }
   TRLTituloList = class(TObjectList)
   protected
      {Retorna o objeto TRLBTitulo que está na posição definida por Index}
      function    GetItem(Index: Integer): TRLBTitulo;
      {Altera o objeto TRLBTitulo que está na posição definida por Index}
      procedure   SetItem(Index: Integer; ATitulo : TRLBTitulo);
   public
      constructor Create;
      {Insere o título no final da coleção}
      function    Add(ATitulo : TRLBTitulo) : integer;
      {Remove da coleção o título}
      function    Remove(ATitulo : TRLBTitulo): Integer;
      {Retorna a posição onde está localizado o título na coleção}
      function    IndexOf(ATitulo : TRLBTitulo): Integer;
      function    FindInstanceOf(AClass: TClass; AExact: Boolean = True; AStartAt: Integer = 0): Integer;
      {Insere o título no final da coleção na posição indicada por Index}
      procedure   Insert(Index: Integer; ATitulo : TRLBTitulo);
   ...
   {@abstract(Representa um conjunto de títulos que serão tratados em arquivos remessa ou retorno)}
   TRLBRemessa = class(TComponent)
   private
      {Nome do arquivo remessa ou retorno (Path completo)}
      fNomeArquivo : string;
      {Número seqüencial do arquivo remessa ou retorno, consulte manual de cobrança do banco}
      fNumeroArquivo : Integer;
      {Data da geração do arquivo remessa ou retorno}
      fDataArquivo : TDateTime;
      fLayoutArquivo: TLayoutArquivo;
      fTipoMovimento: TTipoMovimento;
      {Títulos incluídos no arquivo remessa ou retorno}
      fTitulos : TRLTituloList;
   public
      constructor Create(AOwner: TComponent); override;
      destructor Destroy; override;
      {Lê o arquivo retorno recebido do banco}
      function LerRetorno : boolean;
      {Gerar arquivo remessa para enviar ao banco}
      function GerarRemessa : boolean;
      function GerarRelatorio : TStringList;
...
{Registra os componentes no Delphi paleta RLBoleto}
procedure Register;
{ Tem por objetivo eliminar caracteres inválidos e acrescentar caracteres à esquerda ou
  à direita do texto original para que a string resultante fique com o tamanho desejado}
...
uses RLCob000, RLCob001, RLCob033, RLCob104, RLCob237, RLCob275, RLCob341, RLCob399,
  RLCob409, RLCob356, RLCob748, RLCob756;
...
begin
  ...
  begin
    ...
    txtNumeroBanco2.Caption := txtNumeroBanco1.Caption;
    ...
    txtNumeroBanco3.Caption := txtNumeroBanco1.Caption;
...
procedure TRLBTitulo.InsertRecord;
var
  AAgenciaCodigoCedente,
  ANossoNumero,
  ACarteira,
...
begin
  ...
  begin
    ...
    try
      ...
      if @GetFormatoBoleto <> nil then
        GetFormatoBoleto(Self,AAgenciaCodigoCedente,ANossoNumero,ACarteira, AEspecieDocumento);
      @GetImagem := ABanco.MethodAddress('GetImagem');
      if @GetImagem <> nil then
        ImagemBanco := GetImagem;
      ABanco.Free;
    except
      ABanco.Free;
      raise;
  ...
  begin
    ...
    FCDS.FieldDefs.Add('NumeroBanco'      ,ftString,10);
    FCDS.FieldDefs.Add('ImagemBanco'      ,ftString,10);
    FCDS.FieldDefs.Add('LocalPagamento'   ,ftString,250);
    FCDS.FieldDefs.Add('DataVencimento'   ,ftString,10);
    FCDS.FieldDefs.Add('Referencia'       ,ftString,20);
    FCDS.FieldDefs.Add('NomeCedente'      ,ftString,250);
    FCDS.FieldDefs.Add('CodigoCedente'    ,ftString,20);
    FCDS.FieldDefs.Add('NumeroDocumento'  ,ftString,250);
    FCDS.FieldDefs.Add('DataDocumento'    ,ftString,10);
    FCDS.FieldDefs.Add('EspecieDoc'       ,ftString,10);
    FCDS.FieldDefs.Add('DataProcessamento',ftString,10);
    FCDS.FieldDefs.Add('Aceite'           ,ftString,1);
    FCDS.FieldDefs.Add('NossoNumero'      ,ftString,20);
    FCDS.FieldDefs.Add('Carteira'         ,ftString,20);
    FCDS.FieldDefs.Add('Quantidade'       ,ftString,10);
    FCDS.FieldDefs.Add('ValorMoeda'       ,ftString,20);
    FCDS.FieldDefs.Add('ValorDocumento'   ,ftString,20);
    FCDS.FieldDefs.Add('Instrucoes'       ,ftString,1000);
    FCDS.FieldDefs.Add('Desconto'         ,ftString,20);
    FCDS.FieldDefs.Add('MoraMulta'        ,ftString,20);
    FCDS.FieldDefs.Add('NomeSacado'       ,ftString,250);
    FCDS.FieldDefs.Add('CpfCnpjSacado'    ,ftString,30);
    FCDS.FieldDefs.Add('EnderecoSacado'   ,ftString,250);
    FCDS.FieldDefs.Add('CidadeSacado'     ,ftString,250);
    FCDS.FieldDefs.Add('SacadorAvalista'  ,ftString,250);
    FCDS.FieldDefs.Add('CodigoBaixa'      ,ftString,20);
    FCDS.FieldDefs.Add('LinhaDigitavel'   ,ftString,100);
    FCDS.FieldDefs.Add('CodigoBarra'      ,ftBlob);
    FCDS.FieldDefs.Add('ID'               ,ftString,1000);
    FCDS.CreateDataSet;
  end;

  FCDS.Append;
  FCDS.FieldByName('NumeroBanco').AsString       := IfThen(Cedente.ContaBancaria.Banco.Codigo = '033',Cedente.ContaBancaria.Banco.Codigo,Cedente.ContaBancaria.Banco.Codigo + '-' + Cedente.ContaBancaria.Banco.Digito);
  FCDS.FieldByName('ImagemBanco').AsString       := IntToStr(ImagemBanco);
  FCDS.FieldByName('LocalPagamento').AsString    := AnsiUpperCase(LocalPagamento);
  FCDS.FieldByName('DataVencimento').AsString    := FormatDateTime('dd/mm/yyyy',DataVencimento);
  FCDS.FieldByName('Referencia').AsString        := Referencia;
  FCDS.FieldByName('NomeCedente').AsString       := AnsiUpperCase(Cedente.Nome);
  FCDS.FieldByName('CodigoCedente').AsString     := AAgenciaCodigoCedente;
  FCDS.FieldByName('NumeroDocumento').AsString   := NumeroDocumento;
  FCDS.FieldByName('DataDocumento').AsString     := FormatDateTime('dd/mm/yyyy',DataDocumento);
  FCDS.FieldByName('EspecieDoc').AsString        := AEspecieDocumento;
  FCDS.FieldByName('DataProcessamento').AsString := FormatDateTime('dd/mm/yyyy',DataProcessamento);
  FCDS.FieldByName('Aceite').AsString            := IfThen(AceiteDocumento = adSim,'S','N');
  FCDS.FieldByName('NossoNumero').AsString       := ANossoNumero;
  FCDS.FieldByName('Carteira').AsString          := ACarteira;
  FCDS.FieldByName('Quantidade').AsString        := '';
  FCDS.FieldByName('ValorMoeda').AsString        := '';
  FCDS.FieldByName('ValorDocumento').AsString    :=  FormatCurr('#,##0.00',ValorDocumento);
  FCDS.FieldByName('Instrucoes').AsString        := AInstrucoes.Text;
  FCDS.FieldByName('Desconto').AsString          := '';
  FCDS.FieldByName('MoraMulta').AsString         := '';
  FCDS.FieldByName('NomeSacado').AsString        := AnsiUpperCase(Sacado.Nome);
  case Sacado.TipoInscricao of
    tiPessoaFisica  : FCDS.FieldByName('CpfCnpjSacado').AsString := 'CPF: ' + FormatarComMascara('!000\.000\.000\-00;0; ',Sacado.NumeroCPFCGC);
    tiPessoaJuridica: FCDS.FieldByName('CpfCnpjSacado').AsString := 'CNPJ: '+ FormatarComMascara('!00\.000\.000\/0000\-00;0; ',Sacado.NumeroCPFCGC);
    tiOutro         : FCDS.FieldByName('CpfCnpjSacado').AsString := Sacado.NumeroCPFCGC;
  end;
  FCDS.FieldByName('EnderecoSacado').AsString  := AnsiUpperCase(Sacado.Endereco.Rua + ', ' + Sacado.Endereco.Numero + '   ' + Sacado.Endereco.Complemento);
  FCDS.FieldByName('CidadeSacado').AsString    := AnsiUpperCase(FormatarComMascara('00000-000;0; ',Sacado.Endereco.CEP) + '    ' + Sacado.Endereco.Bairro + '    ' + Sacado.Endereco.Cidade + '    ' + Sacado.Endereco.Estado);
  FCDS.FieldByName('SacadorAvalista').AsString := AnsiUpperCase(Sacado.SacadorAvaLista);
  FCDS.FieldByName('CodigoBaixa').AsString     := ANossoNumero;
  FCDS.FieldByName('LinhaDigitavel').AsString  := CodigoBarra.LinhaDigitavel;
  FCDS.FieldByName('ID').AsString              := ID;
  CodigoBarra.Imagem.Picture.Bitmap.SaveToStream(FStream);
  FStream.Position := 0;
  TBlobField(FCDS.FieldByName('CodigoBarra')).LoadFromStream(FStream);
  FCDS.Post;
  
  FreeAndNil(AInstrucoes);
  FreeAndNil(FStream);
end;

procedure TRLBTitulo.ClearRecords;
begin
  if Assigned(FCDS) then
    FreeAndNil(FCDS);
end;

procedure TRLBTitulo.PreviewModal(num:integer);
var
  ABoleto : TFrmRLBLayout;
  DataSource: TDataSource;
begin
  ABoleto := TFrmRLBLayout.Create(nil);
  try
    DataSource := TDataSource.Create(nil);
    DataSource.DataSet := FCDS;
    if BoletoLayout = blCarne then
    begin
      ABoleto.BoletoCarne.DataSource := DataSource;
      ABoleto.BoletoCarne.PreviewModal;
    end
    else
    begin
      ABoleto.BoletoPadrao.DataSource := DataSource;
      if num = 0 then ABoleto.BoletoPadrao.PreviewModal
      else            ABoleto.BoletoPadrao.SaveToFile('c:\_'+inttostr(num)+'.pdf');
    end;
    FreeAndNil(ABoleto);
    FCDS.EmptyDataSet;
  except
...
begin
  ...
  try
    ...
    begin
      ABoleto.BoletoCarne.DataSource  := DataSource;
      ABoleto.BoletoCarne.PrintDialog := FPrintDialog;
      ABoleto.BoletoCarne.Print;
    end
    else
    begin
      ABoleto.BoletoPadrao.DataSource  := DataSource;
      ABoleto.BoletoPadrao.PrintDialog := FPrintDialog;
      ABoleto.BoletoPadrao.Print;
    end;
    FreeAndNil(ABoleto);
    FCDS.EmptyDataSet;
  except
...
procedure TRLBTitulo.SetVersao(Value: ShortString);
begin
