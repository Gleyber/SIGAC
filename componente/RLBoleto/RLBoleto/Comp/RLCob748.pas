            ...
            begin
               ...
               if StrToInt(Copy(Retorno.Strings[NumeroRegistro],115,2)) <= 69 then
                  ...
                  DataOcorrencia := EncodeDate(StrToInt('19'+Copy(Retorno.Strings[NumeroRegistro],115,2)),
                                     StrToInt(Copy(Retorno.Strings[NumeroRegistro],113,2)),
                                     StrToInt(Copy(Retorno.Strings[NumeroRegistro],111,2)));
               if Trim(Copy(Retorno.Strings[NumeroRegistro],329,8)) = '' then
                 DataCredito := 0
               else
                 DataCredito := StrToDate(Copy(Retorno.Strings[NumeroRegistro],335,2)+'/'+
                                          Copy(Retorno.Strings[NumeroRegistro],333,2)+'/'+
                                          Copy(Retorno.Strings[NumeroRegistro],329,4));
               ...
               ValorRecebido       := StrToFloatDef(Trim(Copy(Retorno.Strings[NumeroRegistro],254,13)),0)/100;
...
begin
  ...
  case StrToIntDef(sMotivoRejeicaoComando,0) of
    00: Result := '00-';
