...
type
  ...
  TProgressEvent = procedure(Sender: TObject; Filename: string;
    Percent: TPercentage; var Continue: boolean) of object;
  TNeedDiskEvent = procedure(Sender: TObject; DiskID: word; var Continue: boolean) of
  object;
  TRestoreFileEvent = procedure(Sender: TObject; var Filename: string;
    FA: integer; var DoRestore: boolean) of object;
  TFileRestoredEvent = procedure(Sender: TObject; var Filename: string;
    var Stream: TFilestream) of object;
  TBackupErrorEvent = procedure(Sender: TObject; const Error: integer; ErrString: string) of
  object;
    ...
    procedure DeCompress(InStream, OutStream: TStream; DoWrite: boolean);
{
  ...
  public
    ...
    function GetArchiveTitleFromStream(Source: TStream;
      var Filelist: TStringList): string;
    ...
    try
      ...
      begin
        ...
        if assigned(fOnProgress) then fOnProgress(self, CurrentFile,
            round((fProgressSize / fSizeTotal) * 100), Continue);
      end;
      ...
      begin
        ...
        if assigned(fOnNeedDisk) then
          ...
          Continue := MessageDlg(Format(cInsertDisk, [IntToStr(ArchiveNumber)]),
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          S := ExpandFilename(ExtractFilepath(pattern) + SR.Name +
            '\' + extractfilename(pattern)) + ' /s';
      ...
      begin
        ...
        repeat
          if assigned(fOnNeedDisk) then
            ...
            Continue := MessageDlg(Format(cInsertDisk, [Disk]),
              mtInformation, mbOKCancel, 0) = mrOk;
...
function TBackupFile.GetArchiveTitleFromStream(Source: TStream;
  var Filelist: TStringList): string;
begin
  if Source <> nil then
    try
...
function TBackupFile.GetArchiveTitle(const Source: string;
  var Filelist: TStringList): string;
var
  L: integer;
  S: string;
...
procedure Register;
begin
  RegisterComponents('Extras', [TBackupFile]);
end;
