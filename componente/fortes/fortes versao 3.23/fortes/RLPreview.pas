...
begin
  if Assigned(fPages) then
    PageNumber:=fPages.FirstPageNumber;
end;

procedure TRLPreview.LastPage;
begin
  if Assigned(fPages) then
    PageNumber:=Max(fPages.FirstPageNumber,fPages.LastPageNumber-(fVisibleBoxes-1));
end;

procedure TRLPreview.NextPage;
begin
  if Assigned(fPages) then
    PageNumber:=Min(fPages.LastPageNumber,PageNumber+fVisibleBoxes);
end;

procedure TRLPreview.PriorPage;
begin
  if Assigned(fPages) then
    PageNumber:=Max(fPages.FirstPageNumber,PageNumber-fVisibleBoxes);
end;
...
procedure TRLPreview.ZoomPage(n:integer);
begin
  if PageNumber<>n then
    PageNumber:=n;
...
begin
  if Assigned(fPages) then
    Result:=fPages.FirstPageNumber+PageIndex
  else
    Result:=0;  
end;
...
begin
  if Assigned(fPages) then
    PageIndex:=aValue-fPages.FirstPageNumber;
end;
...
begin
  inherited;
  //
  fPreview.ZoomPage(fPreview.PageNumber+fPreview.fBoxes.IndexOf(Self));
end;
