...
type
  ...
  {@type TRLDeviceKind - Tipo de dispositivo de impressão.
   ...
   selecionada no diálogo de impressão. Este é o padrão mais recomendado para o Windows, pois o próprio sistema se
   encarregará de descobrir o caminho para o dispositivo através de informações do SO;
  ...
  private
    ...
    procedure   FontSizeBitmapNeeded;
...
begin
  ...
  if Assigned(fOptions) then
    FreeAndNil(fOptions);
  if Assigned(fFontSizeBitmap) then
    FreeAndNil(fFontSizeBitmap);
  //
  inherited;
end;
...
procedure TRLDraftFilter.FontSizeBitmapNeeded;
begin
  if not Assigned(fFontSizeBitmap) then
  begin
    fFontSizeBitmap:=TBitmap.Create;
    fFontSizeBitmap.Width :=1;
    fFontSizeBitmap.Height:=1;
  end;
end;
...
begin
  FontSizeBitmapNeeded;
  fFontSizeBitmap.Canvas.Font.Name:=aFontName;
  fFontSizeBitmap.Canvas.Font.Size:=aSize;
  ...
  IsPitchFixed:=(fFontSizeBitmap.Canvas.TextWidth('.')=fFontSizeBitmap.Canvas.TextWidth('W'));
  if IsPitchFixed then
    OneCharWidth:=fFontSizeBitmap.Canvas.TextWidth(TextSample)/Length(TextSample)
  else
    OneCharWidth:=MagicDelta*fFontSizeBitmap.Canvas.TextWidth(TextSample)/Length(TextSample);
...
begin
  ...
  try
    ...
    try
      ...
      tempbmp.Width :=Round((aRect.Right-aRect.Left)*AspectratioX);
      tempbmp.Height:=Round((aRect.Bottom-aRect.Top)*AspectratioY);
      tempbmp.Canvas.StretchDraw(Rect(0,0,tempbmp.Width,tempbmp.Height),aGraphic);
...
begin
  ...
  begin
    ...
    fOptions.Add(LS_DefaultStr);
    fOptions.Add('80 '+LS_ColsStr);
    fOptions.Add('132 '+LS_ColsStr);
  end;
  //
  Result:=fOptions;
end;
